...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--attention-decoder-nhead",
        type=int,
        default=8,
        help="Number of attention heads in the attention decoder layers.",
    )
...
def get_attention_decoder_model(params: AttributeDict) -> nn.Module:
    if params.num_attention_decoder_layers > 0:
        attention_decoder = AttentionDecoder(
            vocab_size=params.vocab_size,
            decoder_dim=int(params.encoder_dims.split(",")[-1]), # attention_dim/ d_model == 384
            sos_id=params.sos_id,
            eos_id=params.eos_id,
            dim_feedforward=2048,
            num_decoder_layers=params.num_attention_decoder_layers,
            nhead=params.attention_decoder_nhead,
        )
    else:
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, ctc_loss, attention_loss = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
    ...
    loss += params.attention_loss_scale * attention_loss
    ...
    if isinstance(attention_loss, torch.Tensor):
        info["attention_loss"] = attention_loss.detach().cpu().item()
    else:
        info["attention_loss"] = 0
    return loss, info
...
def run(rank, world_size, args):
    ...
    sos_token, eos_token = "<sos/eos>", "<sos/eos>"
    params.sos_id = sp.piece_to_id(sos_token)
    params.eos_id = sp.piece_to_id(eos_token)
    ...
    if model.attention_decoder:
        num_param_attention_decoder =  sum([p.numel() for p in model.attention_decoder.parameters()])
        logging.info(f"Number of attention decoder parameters: {num_param_attention_decoder}")
